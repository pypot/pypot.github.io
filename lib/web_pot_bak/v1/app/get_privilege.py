...
class get_privilege(object) :
    ...
    def GET(self, inner_url):
        ...
        try :
            ...
            #sql = "select * from %s where input_mid=\"%s\"" % (conf.TABLE_NAME_PRIVILEGE, mid)
            sql = "select * from %s" % (conf.TABLE_NAME_PRIVILEGE)
            res = sqlExcutor.execute_query(conf.MYSQL_INFO.get("db_name"), sql)
            if len(res) >= 1:
                for item in res:
                    line = {}
                    ...
                    code = str(item[1])
                    func_type = int(item[2])
                    scan_type = int(item[3])

                    line["code"] = code
                    line["funcType"] = func_type
                    line["scanType"] = scan_type
                    # line["workerID"] = "11"
